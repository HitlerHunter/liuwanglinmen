...
static CGFloat const SGMargin = 16;
...
- (void)layoutSubviews {
    ...
    if (_imageView.image) {
        imageViewW = _imageView.image.size.width * (imageViewH/_imageView.image.size.height);
    }
    
    _imageView.frame = CGRectMake(imageViewX, imageViewY, imageViewW*0.8, imageViewH*0.8);
...
- (void)addTitle:(NSString *)title{
    NSMutableArray *arr = [NSMutableArray arrayWithArray:_titleArray?_titleArray:@[]];
    [arr addObject:title];
    
    _titleArray = arr;
    self.tempArr = [NSArray arrayWithArray:arr];
