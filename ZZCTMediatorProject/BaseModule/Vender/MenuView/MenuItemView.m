...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        arrowMark.centerY = itemView.mj_h/2.;
...
- (void)layoutSubviews{
    ...
    _title.mj_w = self.frame.size.width - 15;
    _title.mj_y = 0;
    _title.mj_x = 2;
...
//    }
    ...
    _arrowMark.centerY = _itemView.mj_h/2.;
...
//    NSArray *titles = @[@"发起群聊",@"添加朋友",@"扫一扫",@"收付款"];
    ...
    CGRect rect = [sender.superview convertRect:sender.frame toView:KeyWindow];
    CGPoint btnRightPoint = CGPointMake(rect.origin.x+sender.width, rect.origin.y+sender.height);
    
    __weak typeof (self) weakself = self;
    [[HEMenu shareManager] showPopMenuSelecteWithFrameWidth:self.mj_w height:30*self.items.count point:btnRightPoint item:_items imgSource:nil action:^(NSInteger index) {
        NSLog(@"%ld", index);
