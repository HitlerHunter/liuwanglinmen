...
#define kItemHeight 30.f       // item 高
...
- (instancetype)initWithFrame:(CGRect)frame menuWidth:(CGFloat)width height:(CGFloat)height point:(CGPoint)point items:(NSArray *)items imgSource:(NSArray *)imgSource action:(void (^)(NSInteger row))action
{
    ...
    if (self) {
        ...
        CGFloat x2 = self.point.x - self.layerWidth ;
...

{
    ...
    CGFloat y1 = self.point.y + kTriangleHeight*2;
    CGFloat y2 = y1 + self.layerHeight-kTriangleHeight;
    CGFloat x1 = self.point.x - kTriangleHeight*3;
    CGFloat x2 = self.point.x - self.layerWidth;
    CGFloat x3 = self.point.x;
    ...
    // 起始点1
    CGContextMoveToPoint(context, self.point.x - kTriangleHeight*2, self.point.y);//设置起点
    // 起始点2
