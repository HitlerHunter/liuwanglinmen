...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.selectBlock = ^(BOOL isSelect) {
        if (isSelect == YES) {
            ...
            weakSelf.globalVar.currentNum ++;
            if (weakSelf.globalVar.currentNum  > 0) {
                weakSelf.previewBtn.enabled = YES;
                weakSelf.selectBtn.enabled = YES;
                weakSelf.selectNumLabel.hidden = NO;
            }
            weakSelf.selectNumLabel.text = [NSString stringWithFormat:@"%d",weakSelf.globalVar.currentNum];
            ...
            if (weakSelf.globalVar.currentNum  == 0) {
                ...
                weakSelf.selectBtn.enabled = NO;
                
                weakSelf.selectNumLabel.hidden = YES;
            }
            if (weakSelf.globalVar.currentNum  >= 0) {
                weakSelf.selectNumLabel.text = [NSString stringWithFormat:@"%d",_globalVar.currentNum];
                [weakSelf deleteCompressionImageIndexPath:(NSIndexPath *)indexPath];
            }
        }
    };
