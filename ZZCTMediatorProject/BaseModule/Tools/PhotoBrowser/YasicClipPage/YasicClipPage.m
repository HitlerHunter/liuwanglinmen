...
- (instancetype)initWithImage:(UIImage *)image{
    self = [super init];
    if (self) {
        self.targetImage = [UIImage fixOrientation:image];
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    self.cropAreaY = (kScreenHeight - self.clipHeight)/2;
    self.cropAreaWidth = self.clipWidth;
    self.cropAreaHeight = self.clipHeight;
    
    self.bigImageView.image = self.targetImage;
    
    UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(30, kScreenHeight-60, 80, 40)];
    ...
    UIButton *sureBtn = [[UIButton alloc] initWithFrame:CGRectMake(kScreenWidth - 110, kScreenHeight-60, 80, 40)];
    [sureBtn setTitle:@"裁剪" forState:UIControlStateNormal];
...
- (void)sureBtnClick{
    if (self.delegate && [self.delegate respondsToSelector:@selector(YasicClipPage:oldImage:newImage:)]) {
        [self.delegate YasicClipPage:self oldImage:self.targetImage newImage:[self cropImage]];
    }
}
